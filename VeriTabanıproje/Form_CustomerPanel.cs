...
namespace VeriTabanıproje
{
    public partial class Form_CustomerPanel : Form
    {
        ...
        private void dataGridView1_RowEnter(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                txtParcaNumarasi.Text = dataGridView1.Rows[e. RowIndex].Cells["ParcaId"].Value.ToString();
                picUrunResmi.ImageLocation = dataGridView1.Rows[e.RowIndex].Cells["ResimNo"].Value.ToString()+ dataGridView1.Rows[e.RowIndex].Cells["IslemeResimNo"].Value.ToString();
                txtUrunIsmi.Text = dataGridView1.Rows[e.RowIndex].Cells["ParcaAdi"].Value.ToString();
            }
            catch
            {
            }
        ...
        {
            foreach(Control control in gbUrunBilgileri.Controls)
            {
        ...
        private void txtArama_TextChanged(object sender, EventArgs e)
        {
            ...
            SqlDataAdapter dataAdapter = new SqlDataAdapter("Parca_Ara", connect);
            dataAdapter.SelectCommand.CommandType = CommandType.StoredProcedure;
            dataAdapter.SelectCommand.Parameters.AddWithValue("@w", txtArama.Text.Trim());
            DataTable dt = new DataTable();
            dataAdapter.Fill(dt);
        ...
        public void SiparisVer(DateTime SiparisTarihi, DateTime TeslimTarihi, string ParcaAdedi,string Dovizturu, string ParcaId,string FirmaId, string YoneticiId, string yukseklik,string en,string cap,string boy,string renk)
        {
            try
            {
                ...
                connect.Open();
                SqlCommand command = new SqlCommand("SiparisVer", connect);
                ...
                SqlCommand command3 = new SqlCommand("Select *from Siparis", connect);
                SqlDataReader datareader = command3.ExecuteReader();
                int SiparisKod = 0;
                while (datareader.Read())
                    SiparisKod = Convert.ToInt32(datareader["SiparisKod"].ToString());
                datareader.Close();

                SqlCommand command2 = new SqlCommand("SiparisVer_Parca", connect);
                command2.CommandType = CommandType.StoredProcedure;
                command2.Parameters.AddWithValue("@SiparisKod", SiparisKod);
                command2.Parameters.AddWithValue("@yukseklik", yukseklik);
                command2.Parameters.AddWithValue("@en", en);
                command2.Parameters.AddWithValue("@boy", boy);
                command2.Parameters.AddWithValue("@cap", cap);
                command2.Parameters.AddWithValue("@renk", renk);
